    ...
    <div className={`card overflow-hidden ${isSelf ? 'ring-2 ring-scp-green' : ''}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {isSelf && (
                <span className="text-xs font-bold bg-scp-green text-scp-navy px-2 py-0.5 rounded-full">You</span>
              )}
            </div>
      ...
      <div className="px-6 py-3 border-b border-scp-gray-warm">
        ...
        <div className="h-2.5 bg-scp-gray-warm rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full ${avatarColor}`}
            style={{ width: `${speaker.share_of_voice}%`, opacity: 0.85 }}
          />
        </div>
      </div>
...
export function ResultsPage() {
  ...
  // Put the self speaker first
  const orderedSpeakers = selfName
    ? [...scores.speakers].sort((a, b) =>
        a.name === selfName ? -1 : b.name === selfName ? 1 : 0
      )
    : scores.speakers

  const activeScore = orderedSpeakers[activeSpeakerIdx] ?? orderedSpeakers[0]

  // Ring progress: 78 out of 100 → dashoffset
        ...
        <div>
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="text-scp-gray-mid text-sm mt-1">
                ...
                {isObserver && (
                  <>
                    <span className="mx-2 text-scp-gray-cool">·</span>
                    <span className="text-scp-gray-mid italic">Observer record</span>
                  </>
                )}
              </p>
            </div>
            <button
              ...
              className="text-scp-blue text-sm font-semibold hover:underline flex-shrink-0"
            >
              How is this scored? →
            </button>
          </div>
        ...
        <div className="bg-scp-navy rounded-xl px-6 py-6 flex items-center gap-6 relative overflow-hidden">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              ...
              <button
                onClick={() => setDrawerOpen(true)}
                className="w-4 h-4 rounded-full bg-white/20 text-white text-xs font-bold flex items-center justify-center hover:bg-white/30 transition-colors"
              >
                ?
              </button>
            </div>
            ...
            <div className="relative w-20 h-20">
              <svg width="80" height="80" viewBox="0 0 80 80" className="-rotate-90">
                ...
                <circle
                  ...
                  stroke="#b5bd00" strokeWidth="7" strokeLinecap="round"
            ...
            {scores.topics.map((topic, i) => (
              <div
                ...
                className={`px-5 py-4 flex items-center gap-4 flex-wrap ${
                  i < scores.topics.length - 1 ? 'border-b border-scp-gray-warm' : ''
                } hover:bg-scp-navy-tint transition-colors`}
              >
                ...
                <div className="flex gap-1.5 flex-wrap">
                  {topic.speakers_involved.map(name => (
                    <span key={name} className="text-xs px-2 py-0.5 bg-scp-navy-tint text-scp-navy rounded-full font-medium">
                      {name}
                    </span>
                  ))}
                </div>
